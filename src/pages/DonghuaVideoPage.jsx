...
import { getDonghuaVideo } from '../api/animeApi'
...
export default function DonghuaVideoPage() {
  ...
  const fetchFn = useCallback(() => {
    ...
    return getDonghuaVideo(episodeUrl)
  }, [episodeUrl])
  ...
  useEffect(() => {
    ...
    addToHistory(user.id, { animeUrl: results.all_episode_url || episodeUrl, animeTitle: title.replace(/\s+Episode\s+\d+.*$/i,'').trim(), episodeUrl, episodeTitle: title, thumbnail: results.thumbnail, type: 'donghua' })
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {loading && <LoadingSpinner text="Memuat video donghua..."/>}
      {error && <ErrorMessage message={error} onRetry={refetch}/>}
      {!loading && !error && results && (
        <div className="animate-fade-in space-y-4">
          <div className="relative bg-black rounded-2xl overflow-hidden border border-red-900/30 aspect-video">
              ...
              <div className="flex flex-wrap gap-2">
                {allServers.map((server, i) => (
                  <button key={i} onClick={() => setActiveServer(server)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border flex items-center gap-1.5 ${activeServer?.index === server.index ? 'bg-red-700 border-red-500 text-white' : 'bg-darkCard border-white/10 text-gray-400 hover:border-red-600/50 hover:text-white'}`}>
          ...
          {(results?.previous_episode_url || results?.next_episode_url) && (
            <div className="flex items-center justify-between gap-3">
              <button onClick={() => results.previous_episode_url && navigate(`/donghua/video?url=${encodeURIComponent(results.previous_episode_url)}`)} disabled={!results.previous_episode_url}
                className="flex items-center gap-2 px-4 py-2.5 bg-darkCard border border-white/10 rounded-xl text-sm text-gray-400 hover:text-white hover:border-red-600/50 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
                <IconChevronLeft size={16}/>Episode Sebelumnya
              </button>
              <button onClick={() => results.next_episode_url && navigate(`/donghua/video?url=${encodeURIComponent(results.next_episode_url)}`)} disabled={!results.next_episode_url}
                className="flex items-center gap-2 px-4 py-2.5 bg-darkCard border border-white/10 rounded-xl text-sm text-gray-400 hover:text-white hover:border-red-600/50 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
